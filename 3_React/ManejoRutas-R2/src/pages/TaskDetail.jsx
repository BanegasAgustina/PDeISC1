...
import { formatLongDate, formatTaskDate } from '../utils/dates'
...
export default function TaskDetail() {
  ...
  return <main className="page narrow-page"><Link className="back-link" to="/"><ArrowLeft size={16}/> Volver a mis tareas</Link><article className="detail-card"><p className="detail-label">TAREA</p><h1>{tarea.titulo}</h1><p className="detail-description">{tarea.descripcion}</p><div className="detail-info"><div><span>Fecha</span><strong><span className="date-long">{formatLongDate(tarea.fechaTarea)}</span><span className="date-short">{formatTaskDate(tarea.fechaTarea)}</span></strong></div><div><span>Creada</span><strong><span className="date-long">{formatLongDate(tarea.fechaCreacion)}</span><span className="date-short">{formatTaskDate(tarea.fechaCreacion)}</span></strong></div><div className="detail-status"><span>Estado</span><span className={`status ${tarea.completada ? 'done' : 'pending'}`}><i></i>{tarea.completada ? 'Completada' : 'Pendiente'}</span></div></div><div className="detail-actions"><button className="button" onClick={() => cambiarEstadoTarea(id)}>{tarea.completada ? <><RotateCcw size={16}/> Marcar como pendiente</> : <><Check size={16}/> Marcar como completada</>}</button><button className="delete-button" onClick={() => setShowDeleteModal(true)}><Trash2 size={16}/> Eliminar</button></div></article><ConfirmModal open={showDeleteModal} title="¿Eliminar tarea?" message={`“${tarea.titulo}” se eliminará definitivamente.`} confirmLabel="Eliminar tarea" onCancel={() => setShowDeleteModal(false)} onConfirm={confirmRemove}/></main>
}
